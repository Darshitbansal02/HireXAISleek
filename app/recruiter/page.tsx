...
const PostJobForm = dynamic(() => import("@/components/recruiter/PostJobForm").then(mod => mod.PostJobForm), {
  loading: () => <div className="p-8 text-center">Loading form...</div>
});

const RecruiterInterviewsSection = dynamic(() => import("@/components/RecruiterInterviewsSection").then(mod => mod.RecruiterInterviewsSection), {
  loading: () => <div className="p-8 text-center">Loading interviews...</div>
});

const ShortlistedCandidatesSection = dynamic(() => import("./shortlisted/page"), {
  loading: () => <div className="p-8 text-center">Loading shortlist...</div>
});
...
export default function RecruiterDashboard() {
  ...
  // Stats state
  const [stats, setStats] = useState<any>({ active_jobs: 0, total_candidates: 0, total_applications: 0 });
  ...
  useEffect(() => {
    if (isAuthenticated) {
      ...
      if (activeTab === "assessments") fetchMyTests();
    }
  ...
  const fetchMyJobs = async () => {
    setJobsLoading(true);
    try {
      const jobs = await apiClient.getRecruiterJobs();
      setMyJobs(jobs);
    } catch (err) {
  ...
  const handleViewApplications = async (jobId: number) => {
    ...
    try {
      const apps = await apiClient.getJobApplications(jobId);
  ...
  return (
    <div className="min-h-screen bg-muted/20">
      ...
      <div className="container max-w-7xl mx-auto px-4 py-8">
        ...
        {activeTab === "post" && (
          <PostJobForm onJobPosted={fetchStats} />
        )}
